<template>
  <v-layout row>
    <v-flex xs3>
      <v-select :items="pList" v-model="province" @change="select('province')"></v-select>
    </v-flex>
    <v-flex xs3>
      <v-select :items="cList" v-model="city" v-show="showCity" @change="select('city')"></v-select>
    </v-flex>
    <v-flex xs3>
      <v-select :items="aList" v-model="area" v-show="showArea" @change="select('area')"> </v-select>
    </v-flex>
  </v-layout>
</template>
...
export default {
  ...
  methods: {
    select (type) {
      ...
      this.cList = []
      this.aList = []
      for (let i = 0; i < this.provinces.length; i++) this.pList.push({text: this.provinces[i]})
      for (let i = 0; i < this.citys.length; i++) this.cList.push({text: this.citys[i]})
      for (let i = 0; i < this.areas.length; i++) this.aList.push({text: this.areas[i]})
    }
  },
  created () {
    // set-default
