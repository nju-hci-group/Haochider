...
def oredr_get():
    ...
    for e in order_s:
        ...
        if e.stata == 0:
            ...
            state = "已取消"
        orders.append(
            {"id": e.oid, "time": e.time, "avatar": rs.photo, "rid": rs.rid,"rname":rs.name, "price": e.cost, "cartItems": orderLists,
...
def res_show():
    ...
    for e in resa:
        ...
        if(mda=="免配送费"):
            ...
            mda=float(mda.split("¥")[1])
        res_data.append({"id":e.rid,"name":e.name,"priceDelivery":mda,"stars":e.stars,"type":random.choice("123456789"),"image":photo})
